...
def handler(event: dict, context) -> dict:
    ...
    # GET /status — состояние ИИ-синхронизации и фонда
    if method == 'GET' and path.endswith('/status'):
        return get_status()

    # GET /proposals — список предложений ИИ
    if method == 'GET' and path.endswith('/proposals'):
        return get_proposals(event.get('queryStringParameters') or {})

    # POST /sync — запуск синхронизации ИИ с анализом системы
    if method == 'POST' and path.endswith('/sync'):
        return run_sync(body)

    # POST /propose — ИИ создаёт предложение по улучшению
    if method == 'POST' and path.endswith('/propose'):
        return create_proposal(body)

    # PUT /proposals/{id}/decide — владелец одобряет/отклоняет
...
def run_sync(body):
    ...
    cur.execute(f'SELECT COALESCE(balance,0) FROM "{SCHEMA}"."egsu_market_tokens" WHERE holder=\'system\' LIMIT 1')
    row = cur.fetchone()
    token_balance = float(row[0]) if row else 0.0
    ...
    if open_incidents > 10:
        ai_proposals.append({
            ...
            'logic_reasoning': f'Анализ: {open_incidents} открытых инцидентов > порога (10). Логика: застаревшие инциденты снижают эффективность мониторинга на 40%.',
            'priority': 'high',
            'tokens_required': 50
        })
    ...
    if security_events > 0:
        ai_proposals.append({
            ...
            'description': f'Обнаружено {security_events} событий безопасности. Рекомендую добавить автоматическую блокировку подозрительных IP после 3 попыток.',
            'logic_reasoning': f'Анализ событий безопасности. Превентивная блокировка снижает риск взлома на 85%.',
            'priority': 'high',
            'tokens_required': 75
        })
    ...
    if unread_notifs > 20:
        ai_proposals.append({
            ...
            'description': f'Накоплено {unread_notifs} непрочитанных уведомлений. Рекомендую добавить группировку и приоритизацию уведомлений.',
            'logic_reasoning': f'Большое количество непрочитанных уведомлений указывает на информационную перегрузку. Группировка снижает нагрузку на оператора.',
            'priority': 'normal',
            'tokens_required': 30
        })
    ...
    ai_proposals.append({
        ...
        'description': 'Рекомендую добавить в обучающую выборку данные из последних инцидентов системы для повышения точности классификации.',
    ...
    if proposals_created > 0:
        email_body = f"""ИИ-система ЕЦСУ завершила анализ и создала {proposals_created} предложений по улучшению.

Требуется ваше одобрение!

Предложения:
""" + '\n'.join([f'• [{p["priority"].upper()}] {p["title"]}' for p in ai_proposals]) + f"""

Фонд развития ИИ пополнен на: {fund_contribution} токенов
Текущий баланс фонда: {new_balance} токенов"""
        send_email_notification(f'Создано {proposals_created} предложений по улучшению — требуется одобрение', email_body)
...
def create_proposal(body):
    """ИИ или my-model создаёт предложение по улучшению"""
...
def decide_proposal(proposal_id: int, body: dict):
    """Владелец одобряет или отклоняет предложение ИИ"""
    ...
    cur.execute(f'''UPDATE "{SCHEMA}"."ai_approval_requests"
        ...
        WHERE proposal_id=%s AND status='waiting' ''', (decision, proposal_id))
...
def collect_tokens(body):
    """Сбор токенов, 10% отчисление в фонд развития ИИ"""
