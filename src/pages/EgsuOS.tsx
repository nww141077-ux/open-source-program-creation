  ...
  const steps = [
    ...
    },
    {
      ...
      items: [
        ...
        "Создайте папку C:\\ECSU-OS и распакуйте туда содержимое ZIP",
      ],
      code: null,
    },
    {
      num: "03",
      title: "Скачать и установить Node.js",
      icon: "Package",
      color: "#f59e0b",
      items: [
        "Откройте браузер и перейдите на сайт: https://nodejs.org",
        "Нажмите зелёную кнопку «LTS» — скачается установщик",
        "Запустите скачанный файл (node-vXX.X.X-x64.msi)",
        "Нажимайте Next → Next → Install → Finish",
        "Проверьте установку — откройте cmd и выполните:",
      ],
      code: "node --version\nnpm --version",
    },
    {
      num: "04",
      title: "Открыть командную строку в нужной папке",
      icon: "Terminal",
      color: "#06b6d4",
      items: [
        "Нажмите Win+R, введите cmd, нажмите Enter",
        "Перейдите в папку с проектом командой:",
        "Убедитесь что вы в правильной папке — список файлов:",
        "Должны быть видны index.html и другие файлы билда",
      ],
      code: "cd C:\\ECSU-OS\ndir",
    },
    {
      ...
      color: "#a855f7",
      items: [
        "Находясь в папке C:\\ECSU-OS выполните команду:",
        "На все вопросы нажимайте Enter (оставляйте по умолчанию)",
        "Появится файл package.json — это конфигурация проекта",
        "АЛЬТЕРНАТИВА: просто скачайте готовый package.json кнопкой выше",
      ],
      code: "npm init -y",
    },
    {
      num: "06",
      title: "Установить Electron",
      icon: "Cpu",
      color: "#10b981",
      items: [
        "Установите Electron в папку проекта (не глобально):",
        "Это займёт 1–3 минуты — качается ~100 МБ",
        "Когда появится строка added X packages — готово",
        "Появится папка node_modules — это нормально, не удалять",
      ],
      code: "npm install electron --save-dev",
    },
    {
      num: "07",
      title: "Добавить main.js",
      icon: "FileCode",
      color: "#ec4899",
      items: [
        "Скачайте готовый main.js кнопкой выше и положите в C:\\ECSU-OS",
        "ИЛИ создайте файл вручную: откройте Блокнот, вставьте код ниже,",
        "сохраните как «main.js» (тип: все файлы) в папку C:\\ECSU-OS",
        "ВАЖНО: файл должен называться main.js, не main.js.txt",
      ],
...
app.on('window-all-closed', () => app.quit());`,
    ...
    {
      num: "08",
      title: "Прописать команду запуска в package.json",
      icon: "Settings",
      color: "#f59e0b",
      items: [
        "Откройте файл C:\\ECSU-OS\\package.json в Блокноте",
        "Найдите строку \"scripts\" и добавьте команду start:",
        "Также добавьте поле \"main\": \"main.js\"",
        "Сохраните файл — АЛЬТЕРНАТИВА: скачайте готовый package.json выше",
      ],
      code: `{
  "name": "ecsu-os",
  "version": "2.0.0",
  ...
  "scripts": {
    ...
    {
      num: "09",
      title: "Первый запуск ECSU OS",
      icon: "Play",
      color: "#00ff87",
      items: [
        "В командной строке, находясь в C:\\ECSU-OS, выполните:",
        "Откроется окно с рабочим столом ECSU OS",
        "Если появилась ошибка «Cannot find module» — повторите шаг 06",
        "Если ошибка «index.html not found» — проверьте что билд распакован в ту же папку",
      ],
      code: "npm start",
    },
    {
      num: "10",
      title: "Создать ярлык для быстрого запуска",
      icon: "Layers",
      color: "#3b82f6",
      items: [
        "Скачайте готовый start-ecsu.bat кнопкой выше и положите в C:\\ECSU-OS",
        "ИЛИ создайте файл start-ecsu.bat вручную с кодом ниже",
        "Щёлкните правой кнопкой по start-ecsu.bat → Отправить → Рабочий стол (создать ярлык)",
        "Теперь ECSU OS запускается двойным кликом по ярлыку",
      ],
      code: `@echo off
cd /d C:\\ECSU-OS
npm start`,
    ...
    {
      num: "11",
      title: "Решение типичных ошибок",
      icon: "AlertTriangle",
      color: "#f43f5e",
      items: [
        "Ошибка «npm не является командой» → Node.js не установлен, повторите шаг 03",
        "Ошибка «Cannot find module electron» → выполните: npm install electron --save-dev",
        "Ошибка «Unable to load preload script» → удалите строку preload из main.js",
        "Чёрный экран вместо приложения → проверьте что index.html есть в папке C:\\ECSU-OS",
        "Окно открылось, но сайт не работает → нужен интернет для загрузки ресурсов",
      ],
      code: "npm install electron --save-dev",
    },
    {
      ...
      color: "#64748b",
      items: [
        "Скачайте новый билд с poehali.dev (Скачать → Скачать билд)",
        "Удалите старые файлы из C:\\ECSU-OS (кроме main.js, package.json, node_modules)",
        "Распакуйте новый ZIP в C:\\ECSU-OS",
        "Запустите npm start — готово, система обновлена",
      ],
      code: null,
    },
  ];
