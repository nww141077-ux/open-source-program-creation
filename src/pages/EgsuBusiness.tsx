...
function SectionPhases() {
  const phases = [
    {
      num: "01",
      label: "Запуск и пилоты",
      period: "0–2 года",
      color: "#f43f5e",
      status: "Отрицательный денежный поток",
      statusColor: "#f43f5e",
      income: ["Гранты и субсидии", "Стартовые подписки ранних партнёров"],
      costs: ["Высокие затраты на разработку", "Маркетинг и найм персонала"],
      focus: "Привлечение первых 3–5 стран/секторов, доказательство концепции",
    },
    {
      num: "02",
      ...
      period: "2–4 года",
      color: "#f59e0b",
      status: "Выход на безубыточность",
      statusColor: "#f59e0b",
      income: ["Рост подписок", "Транзакционные сборы", "Лицензирование ИИ"],
      costs: ["Оптимизация IT-инфраструктуры", "Расширение команды"],
      focus: "Масштабирование платформы, построение партнёрской экосистемы",
    },
    {
      num: "03",
      label: "Устойчивый рост",
      period: "4+ лет",
      color: "#00ff87",
      status: "Положительная прибыль",
      statusColor: "#00ff87",
      income: ["Стабильный поток подписок", "Транзакции", "Партнёрства"],
      costs: ["Снижение доли IT за счёт автоматизации"],
      focus: "Инвестиции в новые направления, выход на глобальный рынок",
    },
  ];
      ...
      {phases.map(({ num, label, period, color, status, statusColor, income, costs, focus }) => (
        <div key={num} className="rounded-2xl p-5" style={{ background: "rgba(255,255,255,0.04)", border: `1px solid ${color}30` }}>
          <div className="flex items-start justify-between mb-4">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-xl flex items-center justify-center font-bold text-black text-sm" style={{ background: color }}>
                {num}
              </div>
              <div>
            ...
            <div className="text-xs font-semibold px-3 py-1 rounded-full" style={{ background: `${statusColor}20`, color: statusColor }}>
              {status}
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3 mb-3">
            <div className="rounded-lg p-3" style={{ background: "rgba(0,255,135,0.06)", border: "1px solid rgba(0,255,135,0.12)" }}>
              ...
              {income.map((i, idx) => (
                <div key={idx} className="flex items-start gap-1.5 text-xs text-white/60 mb-1">
                  <span style={{ color: "#00ff87" }}>+</span>{i}
                </div>
              ))}
            </div>
      ...
      {risks.map(({ risk, color, level, mitigation }) => (
        <div key={risk} className="rounded-xl p-4" style={{ background: "rgba(255,255,255,0.04)", border: `1px solid ${color}25` }}>
          <div className="flex items-start justify-between gap-3 mb-2">
            ...
            <span className="text-xs font-semibold px-2 py-0.5 rounded-full shrink-0" style={{ background: `${color}20`, color }}>
              {level}
            </span>
          </div>
...
function SectionStrategy() {
  const steps = [
    ...
    { n: "2", label: "Устав ЕЦСУ",         desc: "Чёткие правила управления, финансирования, защиты данных" },
      ...
      <div className="space-y-3">
        {steps.map(({ n, label, desc }) => (
          <div key={n} className="flex items-start gap-4 rounded-xl p-4" style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)" }}>
            <div className="w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm shrink-0" style={{ background: "rgba(16,185,129,0.2)", color: "#10b981" }}>
              {n}
            </div>
            <div>
              <div className="text-white font-semibold text-sm">{label}</div>
  ...
  return (
    <div className="min-h-screen font-body" style={{ background: "#060a12" }}>
      ...
      <div className="pt-16 pb-8 flex flex-col md:flex-row min-h-screen">
        {/* Sidebar */}
        <aside
          className="w-full md:w-64 shrink-0 flex md:flex-col gap-2 px-4 py-4 overflow-x-auto md:overflow-x-visible md:sticky md:top-16 md:h-[calc(100vh-4rem)]"
