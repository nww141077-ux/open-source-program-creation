...
Вспомогательные ресурсы системы ECSU 2.0, которые могут быть предоставлены органу:
1. Автоматический сканер инцидентов (GDACS, USGS, OpenAQ, CVE, ReliefWeb, EMSC) — верифицированные данные из международных источников
2. ИИ-правовой анализ — подбор применимых норм, формирование требований к органу
3. Модуль безопасности — журнал кибератак с IP, провайдером, ASN нарушителя и расчётом ущерба
4. Финансовый учёт — реестр штрафов и требований о возмещении с суммами
5. SMS-уведомления — фиксация факта уведомления органа с временной меткой
6. Публичный реестр обращений — уникальный код регистрации для независимой верификации
7. Правовая база — структурированная база норм УК РФ, КоАП, международных конвенций
8. Генератор документов — полный пакет документов в единый архив
...
- Упомянуть, какие вспомогательные ресурсы системы могут быть предоставлены органу для рассмотрения
...
- Длина: 500-700 слов
- В конце подпись: ${OWNER.name}, ${OWNER.system}, ${OWNER.address}`;
    ...
    try {
      ...
      setGeneratedText(`ОФИЦИАЛЬНОЕ ОБРАЩЕНИЕ ГРАЖДАНИНА
Исх. №ECSU-${Date.now().toString(36).toUpperCase()}
...
Настоящим, в соответствии с Конституцией РФ (ст. 33) и ФЗ №59 «О порядке рассмотрения обращений граждан РФ», сообщаю о следующем нарушении:

${form.incident_description}

Место нарушения: ${form.incident_location || "указано в приложении"}

ПРАВОВЫЕ ОСНОВАНИЯ:
${norms.filter(n => n.norm).map((n, i) => `${i + 1}. ${n.norm} — ${n.desc}`).join("\n")}

ТРЕБУЮ:
1. Провести проверку по существу изложенных фактов
2. Принять меры реагирования в установленные законом сроки
3. Предоставить письменный ответ не позднее 30 дней (ФЗ №59, ст. 12)

ВСПОМОГАТЕЛЬНЫЕ РЕСУРСЫ СИСТЕМЫ ECSU 2.0, ПРЕДОСТАВЛЯЕМЫЕ ОРГАНУ:
Система располагает следующими инструментами для содействия в разрешении ситуации:
• Данные международных систем мониторинга (GDACS, USGS, OpenAQ, CVE, ReliefWeb)
• Журнал фиксации событий с временны́ми метками (неизменяемая база данных)
• Правовой анализ применимых норм с ссылками на законодательство РФ и международные конвенции
• При необходимости: журнал кибератак, финансовый расчёт ущерба, SMS-подтверждения
Все материалы предоставляются по запросу органа на электронный адрес: ${OWNER.email}

ПОСЛЕДСТВИЯ БЕЗДЕЙСТВИЯ:
${norms.find(n => n.inaction)?.inaction || "При непринятии мер — жалоба в прокуратуру (ст. 5.59 КоАП РФ — штраф 5000–10000 руб.)"}
